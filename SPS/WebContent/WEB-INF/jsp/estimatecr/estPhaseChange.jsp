...
VIEW ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
</s:if>
<s:elseif test="%{isModify=='true'}">
MODIFY ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
</s:elseif>
<s:elseif test="%{isUndoReject=='true'}">
UNDO REJECTED ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
</s:elseif>
<s:elseif test="%{isPrint=='true'}">
PRINT ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
</s:elseif>
<s:elseif test="%{isApprove=='true'}">
APPROVE/REJECT ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
</s:elseif>
<s:else>NEW ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else></s:else>
</title>
				...
				<tbody>
					...
					<tr>
						...
						<table class="tblContent" border="0" cellpadding="0"
							...
							<tbody>
								<tr>
									<td><s:form id="frmEstimate" name="frmEstimate" action="estphasechange" method="POST"
										...
										<s:hidden id="isConversion" name="isConversion"></s:hidden>
										...
										<s:if test="%{isConversion == 'true'}">
											<s:hidden id="crType" name="crType" value="CC"></s:hidden>
										</s:if>
										<s:else>
											<s:hidden id="crType" name="crType" value="CP"></s:hidden>
										</s:else>
										<table width="75%" class="tldFromContent" border="0">
											...
											<tbody>
												<tr>
													...
													<s:if test="%{isViewOnly == 'true'}">
													VIEW STANDARD ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
													</s:if>
													<s:elseif test="%{isModify=='true'}">
													MODIFY STANDARD ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
													</s:elseif>
													<s:elseif test="%{isUndoReject=='true'}">
													UNDO REJECTED ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
													</s:elseif>
													...
													<s:else>
													NEW STANDARD ESTIMATION <s:if test="%{isConversion == 'true'}">(30-60 CONVERSION)</s:if><s:else>(PHASE CHANGE)</s:else>
													</s:else>
													</td>
												</tr>
												<tr>
												...
												<tr>
													...
													<table width="100%" border="0">
														...
														<tr>
															...
															<td>:</td>
															<td><s:textfield name="lineLength" id="lineLength" onkeyup="show_estcost();numericValidate(this,event);" onblur="show_estcost();add_materials(false)"  readonly="true"/></td>
														</tr>
														<tr>
														...
														<!--  <tr>
															<td>Lb(m)</td>
															<td>:</td>
															<td><s:textfield name="lb"	id="lb" /></td>
														</tr>
														-->
														<tr>
													...
													<td valign="top">
														<table border="0">
															...
															<tr>
																...
																<td>
																	<s:url id="estcost_url" value="estphasechange!calEstCostToLineLen" />
										...
										<table>
											<tr>
												...
												<s:if test="isViewApplicant!=null">
													...
													<s:submit theme="simple" value="Print" id="btnPrint" onclick="javascript:printingPhaseChange()" type="button" ></s:submit>
												</s:if>
												<s:else>
