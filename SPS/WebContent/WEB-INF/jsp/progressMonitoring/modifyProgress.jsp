...
function deleteNorm(lineid,date){
	var status= confirm("Do you really want to remove this ??");
	//alert(""+ lineid  + ""+appRefNoValue + "  " + date);
	 if (status== true){			
		 var estimateNo = $("#estimateNo").val();
			...
			//alert("lineid"+ lineid  + "appRefNoValue"+estimateNo );	
			$.getJSON("smc/deleteLine.ajax?lineid="+lineid+"&estimateNo="+estimateNo+"&dateupdate="+date, {
					ajax_command: "deleteLine",ajax_ieCacheFix:new Date().getTime() },
					function(json){ 
...
function answers()
{
	...
	document.getElementById("estimateNo").value = selectedanswer;
}
...
//alert(json.estimateDetails.length);			
			...
			$("#progressDetails tr:gt(0)").remove();
					...
					//$('#totalcostRow').remove();
							...
							//<!--'<td width=\'25px\' align=\'center\'><image onclick="deleteNorm(\'' + addedMilestone.milesToneId + '\'' +',\'' + addedMilestone.updatedDate + '\'' +',\'' + null+'\')" src="../../image/X.png" /></td>'+-->
							'</tr>';
...
function loadProjectDetails(val){
			document.getElementById("projectNo").value=val.value;
			...
			$.getJSON("smc/loadEstimationDetails.ajax?projectNo="+projectNo, {
			ajax_command: "searchProgressDetailsJobNumber",ajax_ieCacheFix:new Date().getTime() },
			function(json){ 
				if(json.invalid){
				   // invalidStatus = "true";
					alert(json.errorMessage);
...
function saveProgress(){
	...
	if(milestoneId =='-1'){
		alert("Please enter the progress milestone !!!...");
	  ...
	  ///  if(selectedItemsList !=''){
		...
		function(json){ 
					...
					if(json.invalid){	
						...
						var status= confirm("Already Exist.Do you really want to update this ?");
						 if (status== true){
							 $.getJSON("smc/updateProgress.ajax?milestoneId="+milestoneId+"&remark="+remark+"&estimateNo="+estimateNo+"&projectNo="+projectNo+"&dateOfprogress="+dateOfprogress, {
									ajax_command: "updateProgress",ajax_ieCacheFix:new Date().getTime() },
									function(json){ 
																					...
																					<tr>
																						<td>
																							<div class="main">
																									...
																									<table>
																									<tr>
																									<td width="25%">Construction Reference :</td>
                            
              <td width="25%"><s:select name="ConRef" 
														list="listConref" 
												id="listConref" 		 																		
									...
									headerValue="-- Please Select --" onchange="answersJob()"></s:select>
                                																...
                                																<tr>
																									...
																									<s:if test="%{costCenterNo!='510.20'}">
																										...
																										id="jobNumberList" 		 																		
																										headerKey="-1"												
																										headerValue="-- Please Select --" onchange="loadProjectDetails(this);"></s:select>
																										...
																										<tr>
																											...
																											<s:if test="%{loggedInUserLevel=='EE'}">
																											<s:submit theme="simple" align="right" value="Delete All Prograss"	method="delete" id="btnClose" onclick ="return confirm('Are you sure ?')"></s:submit>
																											</s:if> 
