...
clearPeggingScheduleForm();
 ...
 $('#pivDatePicker').datePicker({clickInput:true});
	...
	var pegTreeLoader = new Ext.tree.TreeLoader({
		url: contextPath + '?ajax_command=loadPegInfo&masterId='+pegMasterId // equivalent to dataUrl
	});
...
function LoadpeggingMaster(id){
	pegMasterId = id.value ;
	var contextPath='sps/loadPegInfo.ajax';

	var pegTreeLoader = new Ext.tree.TreeLoader({
		url: contextPath + '?ajax_command=loadPegInfo'
...
function validateEstimateNoFormat(estimateNo){

	var re = "\b\d\d\d\.\d\d\/\d\d\/\d\d\d\d\b";
	//alert(estimateNo);
    return re.test(estimateNo);
	}
...
function addPegItems(){
	...
	//alert(document.getElementById('itemquantity').value);
			...
			$.getJSON("smc/showPegInfo.ajax?selectedPegId="+selectedID+"&selectedPegText="+selectedIDValue+"&quantity="+document.getElementById('itemquantity').value+"&applicationId="+applicationId+"&estimateNo="+estimateNo+"&masterId="+pegMasterId+"&warehouseId="+wareHouseId, {
...
function viewMasterDetails(){
	...
	//alert(selectedID);
    	    if(selectedID !=''){
				$.getJSON("smc/viewMasterDetails.ajax?selectedPegId="+selectedID+"&masterId="+pegMasterId, {
...
clearPeggingScheduleForm();
	 ...
	 $.getJSON("sps/loadDefaultDropDownsValues.ajax?applicationId="+applicationId, {ajax_command:"loadDefaultDropDownsValues",ajax_ieCacheFix:new Date().getTime()}, function(json) {
		...
		<tr>
			...
			<table id="tblInner" class="tblInner" >
				<tbody>
					...
					<tr  class="tblInner">
						<td>
						<s:form id="frmEstimate" name="frmEstimate" action="pegSchedule!saveDetailEstimation" method="POST"
					...
					<!--  	<s:radio name="pegMasteId" id="pegMasteId" list="#{'1':'Master Template 1','2':'Master Template 2'}" onchange="LoadpeggingMaster(this)"  required="true"/><s:hidden id="hiddenPegMasterId" name="hiddenPegMasterId"></s:hidden>
						-->
						</div>
