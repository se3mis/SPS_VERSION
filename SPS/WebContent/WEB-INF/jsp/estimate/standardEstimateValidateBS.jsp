...
function loadApplicationData(val){
		...
		if(val.value=="0"){
			...
			document.getElementById('normsdiv').style.display = 'none';
		  	...
		  	$("#Validate").removeAttr("disabled");//added Gayani 26/10/2013
		...
		function(json){ 
			...
			$('#entryBy').val(json.entryBy);	
			$('#entryDate').val(json.entryDate);
			$('#isApprovable').val(json.isApprovable);
			if($('#isApprovable').val()=="true"){
				//alert("hi");
				//$("#Validate").attr("disabled", "disabled");
				$("#Validate").removeAttr("disabled");
			}

			$('#dgmcomment').val(json.dgmcomment);
			$('#cecomcomment').val(json.cecomcomment);
			$('#cepdcomment').val(json.cepdcomment);
			$('#eecomcomment').val(json.eecomcomment);
			$('#eepdcomment').val(json.eepdcomment);
			$('#escomcomment').val(json.escomcomment);
			...
			$.each(json.addednorms,function(i,normjson){
				...
				var pkgTable1 = '<tr  id="linesecid'+normjson.linesectionid+'">'+		
							...
							'<td align="left"><input type="text" class="required" style="text-align: right" name="quantity'+i+'" id="quantity'+normjson.linesectionid+'"  maxlength="10" size="10" readonly="true" value="'+normjson.quantity+'"  style="text-align: right;color: green;" onchange="onNumChange(this,'+normjson.standardcost+',\'' + normjson.linesectionid + '\')"  disabled="disabled"/></td>'+
...
function answers()
{
	var selectedanswer=document.getElementById("commenttype").value;
	var comment = document.getElementById("recomReason").value;
	document.getElementById("recomReason").value = comment +' ' + selectedanswer;
}
function validateForm()
{
	
	
	 var rejectReason=$("#rejectedReason").val();
		...
		<tr>
			...
			<table id="tblInner" class="tblInner" >
				<tbody>
					...
					<tr>
						<td>
						<s:form id="frmEstimate" name="frmEstimate" action="estimateBS!standardEstiValidate" method="POST"  validate="true" theme="simple">
						  		                       <table>
						  		                       <tr>
												  		<td><h2> </h2></td>
														<td><h2>VIEW COMMENT</h2></td>
														<!-- <td><h2> </h2></td>
														<td><h2>DGM&nbsp;&nbsp;</h2>
														<input name ="dgmcomment" id="dgmcomment" type="hidden" />
														<img src="../../image/Y.png" onclick="alert(document.getElementById('dgmcomment').value);" style="width:25px;height:25px"/>
														</td>-->
														
														<td><h2>CE COM&nbsp;&nbsp;</h2>
														<input name ="cecomcomment" id="cecomcomment" type="hidden" />
														<img src="../../image/Y.png" onclick="alert(document.getElementById('cecomcomment').value);" style="width:25px;height:25px"/>
														</td>
														
														<td><h2>CE P&D&nbsp;&nbsp;</h2>
														<input name ="cepdcomment" id="cepdcomment" type="hidden" />
														<img src="../../image/Y.png" onclick="alert(document.getElementById('cepdcomment').value);" style="width:25px;height:25px"/>
														</td>
														
														<td><h2>EE COM&nbsp;&nbsp;</h2>
														<input name ="eecomcomment" id="eecomcomment" type="hidden" />
														<img src="../../image/Y.png" onclick="alert(document.getElementById('eecomcomment').value);" style="width:25px;height:25px"/>
														</td>
														
														<td><h2>PE P&D&nbsp;&nbsp;</h2>
														<input name ="eepdcomment" id="eepdcomment" type="hidden" />
														<img src="../../image/Y.png" onclick="alert(document.getElementById('eepdcomment').value);" style="width:25px;height:25px"/>
														</td>
														<td><h2>ES COM&nbsp;&nbsp;</h2>
														<input name ="escomcomment" id="escomcomment" type="hidden" />
														<img src="../../image/Y.png" onclick="alert(document.getElementById('escomcomment').value);" style="width:25px;height:25px"/>
														</td>
														</tr>
						  		...
						  		<table width="100%" border="0" class="tldFromContent1">
														...
														<tr>                             
															<td id="tdBotton" colspan="10" align="right">
															<s:submit cssClass="buttonExit" theme="simple"
														...
														<tr height="20">
															...
															<td>
															<input type="text" name="entryBy" id="entryBy" readOnly="true" disabled="disabled"/></td>
															<td>Entry Date</td>
															
															<td><input type="text" name="entryDate" id="entryDate" readOnly="true" size="60" disabled="disabled"/></td>
															<td width="25%">&nbsp;</td>
														...
														<tr height="20">
															...
															<td><input type="hidden" name="secDepositDisplay" id="secDepositDisplay" readOnly="true" disabled="disabled"/>
															<input type="text" name="secDeposit" id="secDeposit" readOnly="true" disabled="disabled"/></td>
														...
														<tr height="20">
															...
															<td><textarea type="text" name="jobDescription" id="jobDescription" disabled="disabled"></textarea></td>
																...
																<td><textarea type="text"  name="recomReason" id="recomReason" cols="40" rows="3"></textarea></td>
																<td width="25%">&nbsp;</td>
														...
														<tr height="20">
															...
															<td><input type="text" name="fundSource" id="fundSource" disabled="disabled"/></td>
															
																<td>Demand</td>
																
																<td><input type="text" name="demand" id="demand" onchange="onDemandChange(this)" disabled="disabled"/></td>
																<td >Comment type</td>
															    <td ><s:select name ="commenttype" id ="commenttype" list="#{'DE':'--Please Select--','Grid Substations':'Grid Substations','Feeder':'Feeder','Loading(%)':'Loading(%)','Loading(A)':'Loading(A)','Type of Conductor':'Type of Conductor'}"    onchange="answers()"/></td>
														...
														<tr height="20">
															...
															<td colspan="5">
															    	...
															    	<s:if test="%{loggedInUserLevel=='CE' || loggedInUserLevel=='DGM' || loggedInUserLevel=='EE'  || loggedInUserLevel=='PE' || loggedInUserLevel=='PCE'}"> 
																		<s:submit theme="simple" value="Recommend" method="recommendEstimate" id="recommend" tooltip="Enter your Name here"></s:submit>
																		<s:submit theme="simple" value="Approve" method="approve" id="Validate" ></s:submit>
														    		</s:if>	
														    		
																    <s:elseif test="%{loggedInUserLevel=='ES'}">
																    	<s:submit theme="simple" value="Recommend" method="recommendEstimate" id="recommend" ></s:submit>
																    	
																    </s:elseif>
															  
																<s:submit theme="simple" value="Reject" method="reject" id="Reject" onclick="return validateForm()" ></s:submit>
																...
																<input type="button"  value="Clear" onclick="clearFormDetails();" disabled="disabled"/>
													...
													<input type="hidden" name="isApprovable"  id="isApprovable" />
													</s:form>
															</td>
														</tr>
													</table>
