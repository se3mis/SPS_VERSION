	...
	}else
		{
			for (i=0; i<document.forms[0].length; i++){
				if(document.forms[0].elements[i].checked == true)
					...
					var urlStr = "/SPS/calendarBS/web/editEntryCMBS?userNam="+usernm+"&descr="+desc+"&timeval="+time+"&apptId="+apptId+"&apptType="+apptType+"&seldate="+seldate+"&costCen="+costCen;
...
function addNewAppointment(){
	...
	if(document.getElementById("selectedDate")){
		...
		document.forms[0].action="/SPS/calendarBS/web/editEntryCMBS?selDate="+selDate;
		document.forms[0].submit();
	}
}
		...
		return false;
			...
			for (i=0; i<document.forms[0].length; i++){
				if(document.forms[0].elements[i].checked == true)
					...
					var urlStr = "/SPS/calendarBS/web/changeStatusCMBS?desc="+desc+"&status="+status+"&apptId="+apptId+"&costCen="+costCen;
