...
public class ChangePivIdNo extends ActionSupport implements SessionAware, ParameterAware {
	...
	public String changePivIdNoAppDirect(){
		setLoggedData();
		Format format=new Format();
		setPivNo("PIV/"+costCenterNo+"/"+"ANC"+"/"+format.getYear()+"/");
		return SUCCESS;
	}
	
	public String changePivIdNoEstDirect(){
		setLoggedData();
		Format format=new Format();
		setPivNo("PIV/"+costCenterNo+"/"+"ENC"+"/"+format.getYear()+"/");
		return SUCCESS;
	}
	
	public String changePivIdNoJobDirect(){
		setLoggedData();
		Format format=new Format();
		setPivNo("PIV/"+costCenterNo+"/"+"JNC"+"/"+format.getYear()+"/");
		return SUCCESS;
	}
	...
	public String findPivNo(){
		try{
			...
			//System.out.println(pivDetailEjb.findById(id)));
			PivDetail pivDetail=pivDetailEjb.findById(id);
	...
	public String change() {
		try{
			...
			PivDetail pivDetail=pivDetailEjb.findById(id);
			if (pivDetail!=null){
				pivDetail.setIdNo(idNoNew);
				pivDetailEjb.updatePivDetail(pivDetail);
				setMessageType(MSG_DONE);
				setErrorMessage("Piv Detail applicant Id has been changed successfully.");
				return "change";
			}else{
				setMessageType(MSG_ERROR);
				setErrorMessage("Piv Detail Id not found");
				return ERROR;
			}
