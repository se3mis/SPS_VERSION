...
public class PivFormBS extends ActionSupport implements SessionAware, ParameterAware{
	private static final Log log = LogFactory.getLog(PivFormBS.class);
   ...
   public void setLoggedData() {
	   System.out.println("Hiii setLoggedData ");
       log.info( getSession());
   ...
   public String getSessionKey(String key) {
	...
	public String save(){
		...
		try{
			...
			log.info("start save()1");
			PivDetail pivDetail=null;
			String newId=null;
			if (state.equals("issueReInsp")){	
				pivDetail=CreatePivDetailModelREINSP();
				log.info(pivDetail);
				newId =ejb.createPivAutoId(pivDetail, "PIV/"+getCostCenterNo()+"/"+ReferenceType.REINSP+"/"+getFormat().getYear()+"/");
				setPivNo(newId);
			}else{
	...
	public String newDirect(){
		try{
			...
			setPivNo("PIV/"+getCostCenterNo()+"/"+PivPrefixType.APP_NEW_BCONN+"/"+getFormat().getYear()+"/xxxx");
	    	...
	    	applicationIdList=ejb.getApplicationIdList(costCenterNo, "BS");
	...
	public String confirmDirect(){
		try{
	    	...
	    	setPath(CONFIRMPATH);
	    	applicationIdList=new ArrayList<String>();
	    	applicationIdList=ejb.getApplicationIdList(costCenterNo,"CP");
	    	setApplicationIdList(applicationIdList);
			return SUCCESS;
		}catch (Exception e) {
			return ERROR;
	...
	public String findByReferenceNo(){
		...
		ApplicantEjb applicantEjb;
		///////PivDetailEjb pivDetailEjb;
		System.out.println("Hii findByReferenceNo");
		try{
			setBankList(getAllBankList());
			applicantEjb=new ApplicantEjb(getSessionKey("region"));
			//////pivDetailEjb=new PivDetailEjb(getSessionKey("region"));
			...
			if (state.equals("new")){
				System.out.println("Hii application");
				try{
				application=applicationEjb.findByAppId(applicationPK);
				}catch(Exception e){
					System.out.println("Hii application" + e.getMessage());
				}
				System.out.println("Hii application 1 " + application);
			}else if (state.equals("issueReInsp")){	
				application=applicationEjb.findByApplicationNo(referenceNo, costCenterNo);
			}
	    	if (application != null) {
				//setPivDetail(pivDetail);
	    		System.out.println("before "+pivNo);
	...
	public String findByReferenceNoRINF(){
		...
		//////PivDetail pivDetail=null;
		//PivDetailDao pivDetailDao=null;
		System.out.println("Hiiiiiiiiii 1");
		ApplicationPK applicationPK=null;
		Application application=null;
		ApplicationEjb applicationEjb=null;
		Applicant applicant=null;
	...
	private void setPivDeatilNew(Application application) {
		...
		if (state.equals("new")){
			System.out.println("app : " + application.getId().getApplicationId()+application.getId().getDeptId());
...
///////////////////////////////////////////////////////////////////
			...
			System.out.println("eeeeeeeeeeeeee 1" + id.getApplicationId());
			System.out.println("eeeeeeeeeeeeee 1" + id);
			id.setApplicationId("514.20/ABS/2011/0006");
			id.setDeptId("514.20");
			try{
			...
			System.out.println("eeeeeeeeeeeeee" + wiringLandDetail);		
...
///////////////////////////////////////////////////////////////////	
    	...
    	setSubTotal(pivDetail.getSubTotal()!=null? pivDetail.getSubTotal().toString():"0");
    	setVat(pivDetail.getVat()!=null? pivDetail.getVat().toString():"0");
    	setGrandTotal(pivDetail.getGrandTotal()!=null? pivDetail.getGrandTotal().toString():"0");
    	setLoanReference(pivDetail.getLoanReference()!=null? pivDetail.getLoanReference().toString():null);
    	setLoanAmount(pivDetail.getLoanAmount()!=null? pivDetail.getLoanAmount().toString():null);
    	setPayDate(pivDetail.getPaidDate()!=null? pivDetail.getPaidDate().toString():null);
    	setChequeDate(pivDetail.getChequeDate()!=null? pivDetail.getChequeDate().toString():null);
