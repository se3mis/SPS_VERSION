...
public class ServiceEstimate extends ActionSupport implements SessionAware, ParameterAware{


	private static final Log log = LogFactory.getLog(ServiceEstimate.class);
	private static final String newPath="Calendar>Service Estimate";
	...
	private List<ListObject> lineDetailsList;
		...
		{		
				...
				if(appc==null){
					...
					if(application!=null){						
						...
						System.out.println("The loan status is............:"+hasAppLoan);
						if(hasAppLoan!=null && hasAppLoan.equalsIgnoreCase("Y"))
						{
							System.out.println("The loan status is............1:");
	...
	public void setControls(){
		try{
			...
			setLineDetailsList(new ArrayList<ListObject>());
			...
			ListObject lineD = new ListObject("Line Detail","Line Detail");
			getLineDetailsList().add(lineD);
	...
	public void setExistingDetails(){
		try{
			...
			if(spserest!=null){
				//DW
				if(spserest.getIsServiceConversion()!=null )
					isServiceConversion = spserest.getIsServiceConversion();
				...
				System.out.println("@@@ conv length "+spserest.getConversionLength());				
				if(wiringType.equals("OH") && spserest.getConversionLength()!=null  && !spserest.getConversionLength().toString().equals("0")){
	...
	public String save(){
		...
		if(wiringType.equals("OH") && conversionLength!=null &&  conversionLength.length()>0){
	...
	private void setLineDetailsList(List<ListObject> lineDetailsList) {
		this.lineDetailsList = lineDetailsList;
	}


	private List<ListObject> getLineDetailsList() {
