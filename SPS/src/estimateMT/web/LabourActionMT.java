...
public class LabourActionMT extends ActionSupport  implements SessionAware, ParameterAware,ServletResponseAware,ServletRequestAware{
					...
					if(spsetpol.getId().getPointType().equalsIgnoreCase("NEW"))
					{
						...
						if(poleLab!=null)
						{
							for (int k=0;k<poleLab.size();k++)
							{
								...
								lg.setCebLabourCost(new BigDecimal(cebLabCost));
								lg.setCebUnitPrice(new BigDecimal(labRate));
								...
								if(selectLabCodeMap.get(lg.getLabourCode())!=null)
								{
									...
									lg.setCebLabourCost(new BigDecimal(newLabCost));
									lg.setItemQty(new BigDecimal(newQty));
								}
						...
						/*if(distanceToServicePlace!=null && distanceToServicePlace.trim().length()>0)
						{
							...
							if(poleTransLab!=null)
							{
								for (int k=0;k<poleTransLab.size();k++)
								{
									...
									lg.setCebLabourCost(new BigDecimal(lg.getUnitLabourHrs().doubleValue()*lg.getItemQty().doubleValue()*labRate));
									lg.setCebUnitPrice(new BigDecimal(labRate));
									if(selectLabCodeMap.get(lg.getLabourCode())!=null)
									{
										LabourGrid tmpLg = (LabourGrid)selectLabCodeMap.get(lg.getLabourCode());
