...
public class EstimatePivForm extends ActionSupport implements SessionAware, ParameterAware{
   ...
   public String getSessionKey(String key) {
	...
	public String execute(){
		...
		pivNo = "PIV/"+costCenterNo+"/"+PivPrefixType.EST_NEW_CONN+"/"+curYear+"/";
		isCostRec = null;
    	return SUCCESS;
    }
	...
	public String generatetc1(){
    	...
    	Calendar cal = Calendar.getInstance();
		int curYear = cal.get(Calendar.YEAR);
		estimateNo = costCenterNo+"/"+PivPrefixType.EST_TEMP_CONN+"/"+curYear+"/";
		setDefaultsFromMainMenu();
    	return SUCCESS;
    }
	...
	public String generateCR(){
    	...
    	Calendar cal = Calendar.getInstance();
		int curYear = cal.get(Calendar.YEAR);
		estimateNo = costCenterNo+"/"+PivPrefixType.EST_CR_JOBS+"/"+curYear+"/";
		setDefaultsFromMainMenu();
    	return SUCCESS;
    }
			...
			{//piv print or modify
    			...
    			else
    			{
    				...
    				//{
						...
						setEstimateData(speststd,false);
						setPIVData(piv);
						this.isPivView = "true";
    					return SUCCESS;
    				/*}
					...
					else
					{
				        ...
				        if(canGeneratePIV)
				        {
							...
							if(hasAppLoan!=null && hasAppLoan.equalsIgnoreCase("Y"))
							{
								...
								if(speststd.getTotalCost().doubleValue()<loanMax)
								{
								...
								else
								{
									...
									String msg = setEstimateData(speststd,true);
									if(msg!=null)
									{
										setErrorMessage(msg);
										return ERROR;
	...
	public String save(){
		...
		try
		{
			...
			Calendar cal = Calendar.getInstance();
			String pivNoPrefix = "PIV/"+costCenterNo+"/"+PivPrefixType.EST_NEW_CONN+"/"+cal.get(Calendar.YEAR)+"/";
			if(isRevise!=null && isRevise.equals("true"))
	...
	private String setEstimateData(Speststd speststd, boolean haveLoan)
	{
		String msg = null;
		...
		if(speststd.getFixedCost()!=null)
			fixedCost = speststd.getFixedCost().doubleValue();
		double secDeposit = 0;
		if(speststd.getSecurityDeposit()!=null)
			secDeposit = speststd.getSecurityDeposit().doubleValue();
		double addlDeposit = 0;
		if(speststd.getAddlDeposit()!=null)
			addlDeposit = speststd.getAddlDeposit().doubleValue();
		double variableCost = 0;
		if(speststd.getVariableCost()!=null)
			variableCost = speststd.getVariableCost().doubleValue();
		double othCost = 0;
		if(speststd.getOtherCost()!=null)
			othCost = speststd.getOtherCost().doubleValue();
		double labourCost = 0;
		if(speststd.getLabourCost()!=null)
			labourCost = speststd.getLabourCost().doubleValue();
		double transportCost = 0;
		if(speststd.getTransportCost()!=null)
			transportCost = speststd.getTransportCost().doubleValue();
		double overheadCost = 0;
		if(speststd.getOverheadCost()!=null)
			overheadCost = speststd.getOverheadCost().doubleValue();
		double othLabCost = 0;
		if(speststd.getOtherLabourCost()!=null)
			othLabCost = speststd.getOtherLabourCost().doubleValue();
		double dmgCost = 0;
		if(speststd.getDamageCost()!=null)
			dmgCost = speststd.getDamageCost().doubleValue();
		double capitalCost = 0;
		if(speststd.getCapitalCost()!=null)
			capitalCost = speststd.getCapitalCost().doubleValue();
		double convCost = 0;
		if(speststd.getConversionCost()!=null)
			convCost = speststd.getConversionCost().doubleValue();
		double contingencyCost = 0;
		if(speststd.getContingencyCost()!=null)
			contingencyCost = speststd.getContingencyCost().doubleValue();
