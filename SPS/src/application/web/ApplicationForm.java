...
public class ApplicationForm extends ActionSupport implements SessionAware,
	...
	public String execute() {
		System.out.println("##########################################khgjhgh"+state);
		if(state.equals("print")){
			setLoggedData();
			setListTariffCode(getTafiffCode());
			setListTariffCatCode(getTafiffCaTCode());
			return SUCCESS;
		}else if(state.equals("billingClerk")){
			setLoggedData();
			setListTariffCode(getTafiffCode());
			setListTariffCatCode(getTafiffCaTCode());
			findAppId();
	...
	public String save() {
		try {
			...
			String appId=ejb.save(application, wiringLandDetail, getCostCenterNo()+"/"+PivPrefixType.APP_NEW_CONN+"/"+getFormat().getYear()+"/");//prefix
	...
	public String confirm() {
		...
		System.out.println("getApplicationIdPrefix()" + getApplicationIdPrefix());
...
public String	newDirect(){
	...
	*/
	...
	setLoggedData();
	setFormat(new Format());
	System.out.println("hii :" +PivPrefixType.APP_NEW_CONN );
    setPrefix(getCostCenterNo()+"/"+PivPrefixType.APP_NEW_CONN+"/"+getFormat().getYear()+"/");
	setApplicationId(getPrefix()+"xxxx");
...
public String changeServiceAdd(){
	try{
		...
		wiringLandDetail.setNeighboursAccNo(neighboursAccNo);
...
private Application updateConfirmBy(Application application) throws NumberFormatException {
	...
	private Application createApplicationObject ()throws NumberFormatException {
		...
		//log.info("4");
		//Duration
		//System.out.println("duration MESSEGE " + duration);
		//application.setDuration(new BigDecimal(duration));//gayani09
