...
public class ApplicationTC extends ActionSupport implements SessionAware,
		ParameterAware {
	// Common Fields
	...
	private static final String newPath="Temp Conn>Application>New Application";
	private static final String viewPath="Temp Conn>Application>View Application";
	private static final String modifyPath="Temp Conn>Application>Modify Application";
	private static final String confirmPath="Temp Conn>Application>Generate App No";	
	//
	...
	public String save() {
		...
		try {
			...
			if (getFromDate().length()>9){
				...
				System.out.println(getApplicationType()+" "+getApplicationSubType());
				Application application=createApplicationObject();
				WiringLandDetail wiringLandDetail=createWiringLandObject();
				//ejb.save( application, wiringLandDetail);
				//System.out.println("TC "+getCostCenterNo()+"/"+PivPrefixType.APP_TEMP_CONN+"/"+getFormat().getYear()+"/"+"   "+  cityMap.mapCity(costCenterNo, serviceCity)+"/"+systemType+getApplicationType()+"/"+getFormat().getYear()+"/" );
				setApplicationId(ejb.saveANDConfirm(application, wiringLandDetail, getCostCenterNo()+"/"+PivPrefixType.APP_TEMP_CONN+"/"+getFormat().getYear()+"/", costCenterNo+"/"+systemType+getApplicationType()+"/"+getFormat().getYear()+"/" ));
				...
				setErrorMessage("ERROR:-From Date must be filled out.");
...
private Application updateConfirmBy(Application application) throws NumberFormatException {
	...
	private Application createApplicationObject ()throws NumberFormatException {
		...
		application.setApplicationSubType(getApplicationSubType());
