...
public class ReviseJobBS  extends ActionSupport implements SessionAware, ParameterAware{
	...
	public void loadJobNumbersUpdate(){
		try{
			setLoggedData();
			List<String> jobNos = null;
			PcesthmtEjb pcesthmtEjb = new PcesthmtEjb(getRegion());
			...
			statuses.add(new Long(EstimateStatus.JOB_RIVISION));
			statuses.add(new Long(EstimateStatus.JOB_REJECTED));
			statuses.add(new Long(EstimateStatus.UPDATED_REVISED_JOB));
	...
	public String updateRevise(){
		...
		String branchType= (String) request.getSession().getAttribute("branchType");
			...
			if(dto != null){
				Pcesthmt pchmt = dto.getHmt();
				PcesthttEjb httEjb = new PcesthttEjb(region);
				long maxRevNo = httEjb.getMaxHttRevNo(pchmt.getId().getEstimateNo().trim(), costCenterNo,region);
				//gayani
				long newRevNo = maxRevNo ;
