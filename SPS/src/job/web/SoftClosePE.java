...
public class SoftClosePE  extends ActionSupport implements SessionAware, ParameterAware{
	...
	public String rejectPE()
	{
		...
		setErrorMessage("Job Sent To EE For Validation Successfully.");
	...
	{   System.out.println("Hii softClosePE ");
		setLoggedData();
		HttpServletRequest request = ServletActionContext.getRequest();
	...
	{   System.out.println("Hii softClosePE ");
		...
		Long preStatus = pcesthmt.getStatus();
		pcesthmt.setStatus(new Long(EstimateStatus.JOB_SOFT_CLOSEDPE));
		pcesthmt.setAprUid3(getSessionKey("userName"));
		pcesthmt.setAprDt3(new Date());
		pcesthmt.setRejectReason(rejectReason);
		try
		{
			//pcesthmtEjb.updatePcesthmt(pcesthmt);	
			...
			approval.setReferenceNo(projectNo);
			approval.setDeptId(costCenterNo);
			approval.setApprovalType("JOB_SCPE");
			approval.setApprovedLevel(getSessionKey("userRole"));
			approval.setApprovedBy(getSessionKey("userName"));
			approval.setApprovedDate(calendar.getTime());
			approval.setApprovedTime(format.FormatTime());
		...
		loadJobNumbers();
		setMessageType(MSG_DONE);
		setErrorMessage("Job Sent To Accountant For Approval Successfully.");
