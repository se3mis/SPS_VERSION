...
public class JobDetailsBS  extends ActionSupport implements SessionAware, ParameterAware{
	...
	public String post()
	{
		try
		{
			...
			approval.setToStatus(new BigDecimal(EstimateStatus.JOB_APPROVAL_EE).toString());
			...
			pcesthmt.setStatus(new Long(EstimateStatus.JOB_APPROVAL_EE));
			pcesthmtEjb.validateJob(pcesthmt, approval);
			
			setMessageType(MSG_DONE);
			setErrorMessage("DONE:- Job sent for approval successfully!");
	...
	public String approve()
	{
		...
		try{
			...
			setErrorMessage(null);
			String msg = "";
			msg = pcesthmtEjb.approveJob(projectNo, costCenterNo, getSession().get("userRole").toString(),getSession().get("userName").toString(),null);
			//setFormData();
			//clearFields();
	...
	public void loadJobNumbers(){
		try{
			...
			List<String> jobsRevisedUpdated = null;
			...
			status.add(new Long(EstimateStatus.UPDATED_REVISED_JOB));
			...
			//System.out.println(EstimateStatus.);
			//jobsOngoing = pcesthmtEjb.findJobNoList(getCostCenterNo(),new Long(EstimateStatus.JOB_ONGOING)); 
			//jobsRevised = pcesthmtEjb.findJobNoList(getCostCenterNo(),new Long(EstimateStatus.JOB_RIVISION)); 
			jobNos = pcesthmtEjb.findJobNosList(getCostCenterNo(),status); 
			//jobs = pcesthmtEjb.getAll();
	...
	private List<String> getAllJobNumbers(List<Pcesthmt> jobNos){
		List<String> jobs = new ArrayList<String>();
		for(Pcesthmt job : jobNos){
			jobs.add(job.getProjectNo());
		}
		return jobs;
	...
	private List<Pcestdmt> populatePcestdmts(String estimateNo,String costCenter,List<DetailEstimateDTO> detaList){
		...
		if(detaList != null){
			for(DetailEstimateDTO dto :detaList){
				...
				pcestdmtPk.setEstimateNo(estimateNo.trim());
		...
		try{
			...
			PivDetailEjb pivejb =new PivDetailEjb(region);
				...
				if(resourceMap != null && resourceMap.size() > 0){
					delist = populatePcestdmts(costCenterNo,new ArrayList<DetailEstimateDTO>(resourceMap.values()));
				}
				...
				pcesthmtPK.setEstimateNo(getProjectNo().trim());
				pcesthmtPK.setDeptId(costCenterNo);
				pcesthmtPK.setRevNo(2);//for new methods-
				
				Pcesthmt pcesthmt = new Pcesthmt();
				pcesthmt.setId(pcesthmtPK);
				
				pcesthmt.setPartialPmt("F");
				pcesthmt.setNormDefault(new BigDecimal(1));
				pcesthmt.setSubCont("T");
				pcesthmt.setControlled("T");
				pcesthmt.setPriority(new BigDecimal(0));
				pcesthmt.setAllocPaid(new BigDecimal(0));
				pcesthmt.setEstType(new BigDecimal(2));
				pcesthmt.setAllocSettle(new BigDecimal(0)); 
				if(getRebate() == null || getRebate().equalsIgnoreCase(Constants.DEFAULT_STRING)){
					pcesthmt.setPartialAmt(new BigDecimal(0));
					setRebate("0");
				}else{
					pcesthmt.setPartialAmt(new BigDecimal(getRebate()));
				}
				
				
				pcesthmt.setEtimateDt(new java.util.Date());
				pcesthmt.setStatus(19L);
				pcesthmt.setEntBy(userId);
				pcesthmt.setEntDt(new java.util.Date());
				pcesthmt.setCatCd(getCategoryId());
				
				//pcesthtt.set
				pcesthmt.setConfDt(format.getDefaultDate());
				pcesthmt.setAprDt1(format.getDefaultDate());
				pcesthmt.setAprDt2(format.getDefaultDate());
				pcesthmt.setAprDt3(format.getDefaultDate());
				pcesthmt.setAprDt4(format.getDefaultDate());
				pcesthmt.setAprDt5(format.getDefaultDate());
				pcesthmt.setRejctDt(format.getDefaultDate());
				pcesthmt.setReviseDt(format.getDefaultDate());
				pcesthmt.setDescr(getDescription());
				pcesthmt.setProjectNo(getProjectNo());
				//pcesthtt.setStdCost(new BigDecimal(Double.toString(nf.parse(getTotalCost()).doubleValue())));
				if(getTotalCost() == null || getTotalCost().equalsIgnoreCase(Constants.DEFAULT_STRING)){
					setTotalCost("0");
				}
				pcesthmt.setStdCost(new BigDecimal(getTotalCost()));
				
				if(getFundid()!=null && getFundsou().length()>0)
				{
					
					pcesthmt.setFundSource(getFundsou().trim());
					pcesthmt.setFundId(getFundid().trim());
				}
				TempTb tb = new TempTb();
				...
				//estimateEjb.insertDetailEst(pcesthtt,delist,tb,pegItemlist,getFileRef(),region);
				jobEjb.insert(pcesthmt, null, delist);
				...
				//estimateEjb.updateDetailEstimateCost(getProjectNo(),estimateType, costCenterNo, region);
				jobEjb.updateRevisedEstimateCost(pcesthmt,smcType, costCenterNo);
				
				setMessageType(MSG_DONE);
				setErrorMessage("Successfully saved job Details");
				//estimateEjb.addAdditionalCost(getEstimateNo(),costCenterNo,getTotalCost(),region);
				clearForm();
...
private List<Pcestdmt> populatePcestdmts(String costCenter,List<DetailEstimateDTO> detaList){
		...
		if(detaList != null){
			for(DetailEstimateDTO dto :detaList){
				...
				PcestdmtPK pcestdmtPk = new PcestdmtPK();
				pcestdmtPk.setDeptId(costCenter);
				pcestdmtPk.setEstimateNo(getEstimateNo().trim());
				pcestdmtPk.setResCd(dto.getResourceCode());
				...
				//pcestdtt.setResCat(new BigDecimal(dto.getResCategory()));
				pcestdmt.setEstimateCost((dto.getEstimateCost() == null || dto.getEstimateCost().equals(Constants.DEFAULT_STRING) ? new BigDecimal("0") : dto.getEstimateCost()));
				pcestdmt.setEstimateQty((dto.getEstimatedQuantity() == null || dto.getEstimatedQuantity().equals(Constants.DEFAULT_STRING) ? new BigDecimal("0") : dto.getEstimatedQuantity()));
				pcestdmt.setResType(dto.getResourceType());
				pcestdmt.setUom(dto.getUom());
				pcestdmt.setUnitPrice(dto.getUnitPrice());
				pcestdmt.setNormDefault(normDefault);
	...
	public String updateJob(){
		...
		String smcType= (String) request.getSession().getAttribute("smcType");
			...
			if(dto != null){
				...
				if(resourceMap != null && resourceMap.size() > 0){
					addedList = populatePcestdmts(costCenterNo,new ArrayList<DetailEstimateDTO>(resourceMap.values()));
				}
	...
	*/
	...
	public String PrintReviseEstimateReport(){
		...
		String fileSummary = rept.generateReport("EstimateRevise",param,getSession(),REPORT_DIRECTORY,EXPORT_REPORT_DIRECTORY);
	...
	public String PrintOriginalEstimateReport(){
		HttpServletRequest request = ServletActionContext.getRequest();
		...
		String fileSummary = rept.generateReport("EstimateOriginal",param,getSession(),REPORT_DIRECTORY,EXPORT_REPORT_DIRECTORY);
