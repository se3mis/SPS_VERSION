...
public class JournalJobMatAction extends ActionSupport  implements SessionAware, ParameterAware,ServletResponseAware,ServletRequestAware{
	...
	private String[] txtComQty;
	private String[] txtSelComCost;
	...
	public String addMaterials()
	{
		...
		System.out.println("revise add mats   vvvvv");
		try{
				...
				selectMatList = new ArrayList();
					...
					if(selectMatCodeMap!=null && selectMatCodeMap.size()>0){
						...
						while(it.hasNext()){
							...
							String comQty ="";
							if(tmpLst.getCommittedQty()!=null) 
								comQty = tmpLst.getCommittedQty().toString();
							
							String comCost = "0";
							if(tmpLst.getCommittedCost()!=null) 
								comCost = tmpLst.getCommittedCost().toString();
							
							
							if(txtComQty!=null && txtComQty.length>0)
								comQty = txtComQty[i];
							if(txtSelComCost!=null && txtSelComCost.length>0)
								comCost = txtSelComCost[i];
							
							
							if(comQty!=null && comQty.length()>0)
								tmpLst.setCommittedQty(new BigDecimal(comQty));
							if(comCost!=null && comCost.length()>0){
								tmpLst.setCommittedCost(new BigDecimal(nf.parse(comCost).doubleValue()));
								//totalCost = totalCost+nf.parse(cost).doubleValue();
							}
				...
				for(int i=0;i<chkSelMatCode.length;i++)
				{
					...
					if(isNewEst!=null && isNewEst.equals("N"))
					{
						if(savedMat.contains(matCode))
						{
							...
							//System.out.println(jobNo+" "+costCenterNo+" "+pcestdmtPK.getResCd().trim());
							//System.out.println(ejb.isMaterialIssued(jobNo, costCenterNo, pcestdmtPK.getResCd().trim()));
							//JVif(!ejb.isMaterialIssued(jobNo, costCenterNo, pcestdmtPK.getResCd().trim()))
	...
	public void setSavedCusQty(String[] savedCusQty) {
		this.savedCusQty = savedCusQty;
	}



	public String[] getSavedCusQty() {
		return savedCusQty;
	}
